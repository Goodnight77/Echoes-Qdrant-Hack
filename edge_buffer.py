"""Qdrant Edge write buffer — fast ingest layer on top of embedded Qdrant.

Why: the qdrant-edge-demo project showed that a mutable Edge shard (no HNSW
index) accepts writes faster than the main indexed collection. On startup
we replay any pending Edge points into the main Qdrant store, then clear
the Edge shard. This makes bulk indexing feel snappy while the main
HNSW index builds in the background.
