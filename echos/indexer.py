...
def _photo_gps(path: str | Path) -> tuple[float, float] | None:
    """Extract GPS coordinates from EXIF, if present.
    Returns (lat, lon) or None. Converts DMS rationals to decimal degrees."""
    try:
        from PIL import Image
        ...
        with Image.open(path) as im:
            ...
            if not gps_ifd:
                return None
            # GPS tags: 1=N, 2=lat, 3=W, 4=lon
            def _dms_to_decimal(values):
                """Convert (deg, min, sec) rationals to decimal degrees."""
                d = float(values[0])
                m = float(values[1]) / 60.0 if len(values) > 1 else 0
                s = float(values[2]) / 3600.0 if len(values) > 2 else 0
                return d + m + s
            lat = gps_ifd.get(2)  # latitude DMS
            lon = gps_ifd.get(4)  # longitude DMS
